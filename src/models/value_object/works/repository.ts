...
class RepositoryError extends ValueObjectError<string> {
    constructor(
...
export default class Repository extends BaseValueObject<string> {
    validate(const_val: string): string {
        if (const_val.length === 0)       throw new RepositoryError(const_val, "空文字です");
        
        const url_pattern = /^https:\/\/github\.com\/[^\/]+\/[^\/]+$/;
        if (!url_pattern.test(const_val)) throw new RepositoryError(const_val, "GitHubのURL形式ではありません");
